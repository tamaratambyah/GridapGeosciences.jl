...
queue_info = Dict{Symbol,Any}(
  :normal => ("normal",4,"16:00:00",48,1),
  :hugemem => ("hugemem",30,"18:00:00",48,1),
...
function jobdict(name,params,q)
  ...
  project = haskey(ENV,"PROJECT") ? ENV["PROJECT"] : "bt62"
  ...
  Dict(
    ...
    "Dc" =>params[:Dc], # dimension of model: 2D vs. 3D
    "order" => params[:order],
    "dir" => params[:dir],
...
function generate_dictionaries(orders,nlevels,nprocs,Dcs)
  dicts = OrderedDict[]
  for order in orders
    for Dc in Dcs
      for (nl,np) in zip(nlevels,nprocs)
            aux = OrderedDict{Symbol,Any}(
                        ...
                        :driver => driver,
                        :np => np,  # number of processors
                        :Dc => Dc,  # number of model: 2D vs. 3D
                      )
            push!(dicts,aux)
          end
        end
...
jobsdir = projectdir("wave_jobs_3D")
!isdir(jobsdir) && mkdir(jobsdir)

dir = datadir("WaveConvergence_3D")
!isdir(dir) && mkdir(dir)

wdir = projectdir("test/Geophysical")
driver = wdir*"/WaveEquation.jl"


# queue = :normal
# orders = [1,2]
# nlevels = [1,2,3,4,5] # n_ref cubed sphere mesh
# nprocs  = [4,4,16,32,48]
# Dcs = [2]

queue = :hugemem
orders = [1,2]
nlevels = [4,5] # n_ref cubed sphere mesh
nprocs  = [96,96]
Dcs = [3]

dicts = generate_dictionaries(orders,nlevels,nprocs,Dcs)
