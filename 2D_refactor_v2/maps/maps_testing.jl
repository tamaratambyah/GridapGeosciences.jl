...
struct PanelMap{T} <: Map # rotate panel 1 -> panel p
  pid::T
end

function Gridap.Arrays.return_cache(f::PanelMap,X::VectorValue{3})
  panel_id = f.pid
...
function Gridap.Arrays.evaluate!(cache,f::PanelMap,X::VectorValue{3})
  y, A = cache
  y = A⋅ X
end


struct InvPanelMap{T} <: Map # rotate panel p -> panel 1
  pid::T
end

function Gridap.Arrays.return_cache(f::InvPanelMap,X::VectorValue{3})
  panel_id = f.pid
...
function Gridap.Arrays.evaluate!(cache,f::InvPanelMap,X::VectorValue{3})
  y, A = cache
  y = A⋅ X
end
...
# panelMap = lazy_map(PanelMap(panel_id), Xs)
...
# invPanelMap = lazy_map(InvPanelMap(panel_id), Xs)
...
# Master = Gridap.Arrays.Operation(InvPanelMap(panel_id))(PanelMap(panel_id))
# z = lazy_map(Master,Xs)
# cache = array_cache(z)
# bm3() = lazy_collect(cache,z)
# @benchmark bm3()
...
# struct OtherPanelMap <: Map # rotate panel 1 -> panel p
# end

# function Gridap.Arrays.return_cache(f::OtherPanelMap,X::VectorValue{3},panel_id::Int64)
#   y = VectorValue(get_array(X)) # data from X into y
#   A =  TensorValue( rotate_panel_1_to_p[panel_id] ) # rotate panel 1 -> panel p
#   return y, A
# end

# function Gridap.Arrays.evaluate!(cache,f::OtherPanelMap,X::VectorValue{3},panel_id::Int64)
#   y, A = cache
#   y = A⋅ X
# end
...
# Xs =  [Point(1.0,1.0,1.0) for i in 1:1000]
# otherPanelMap = lazy_map(OtherPanelMap(), Xs,panel_id)
# cache = array_cache(otherPanelMap)
# bm4() = lazy_collect(cache,otherPanelMap)
# @benchmark bm4()
