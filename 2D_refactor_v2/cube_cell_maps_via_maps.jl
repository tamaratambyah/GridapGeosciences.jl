...
"""
struct PanelMap{T} <: Map
  rotate_panel_p_to_1::T
  rotate_panel_1_to_p::T
end
### why is the cache not working? Ask Jordi...
function Gridap.Arrays.return_cache(cache,f::PanelMap,X::VectorValue{3},panel_id::Int,inverse::Bool=false)
 y = zero(X)
 A = similar(f.rotate_panel_1_to_p[panel_id])
 println(y,A)
 return y,A
end
...
_panelMap = lazy_map(PanelMap(rotate_panel_p_to_1,rotate_panel_1_to_p), Xs,panel_id)
cache = array_cache(_panelMap)
bm1() = lazy_collect(cache,_panelMap)
@benchmark bm1()
