...
evaluate(cmaps_2D[cell_id],ref_coord) # zero (as expected)

_geo_map(p) = x ->  panel_map(p,bump_panel1(x))
f = lazy_map(_geo_map,panel_ids)
...
ref_cube_model_2D = Gridap.Adaptivity.refine(cube_model_2D)
ref_cube_model_3D = Gridap.Adaptivity.refine(cube_model_3D)

panel_ids = get_panel_ids(ref_cube_model_2D)


ref_coord = Point(1,1) # point on reference FE (0,0)->(1,1)
cell_id = 23 # for testing

cmaps_2D = get_cell_map(ref_cube_model_2D)
ncells = num_cells(ref_cube_model_2D)

evaluate(cmaps_2D[cell_id],ref_coord) # zero (as expected)
f = lazy_map(_geo_map,panel_ids)


panel1_cell_ids = findall(x->x==1, panel_ids) # id of cells in panel 1
panel1_maps = cmaps_2D[panel1_cell_ids] # maps of those cells
rep_cell_1 = repeat(panel1_maps,3) # repeat for 3 panel
_cell_map = lazy_append(rep_cell_1,rep_cell_1) # lazy append to get 6 panels
...
# test against the 3D cells which are 'correct'
cmaps_3D = get_cell_map(ref_cube_model_3D)
ref_cell_coords = get_cell_ref_coordinates(get_grid(ref_cube_model_2D))
