...
function adapt_model(model::Parametric3DOctreeDistributedDiscreteModel)
  href_model = GridapGeosciences.Distributed.horizontally_uniformly_refine(model)
  return GridapGeosciences.Distributed.vertically_uniformly_refine(href_model)
end
...
writevtk(Ωh,dir*"/prolongation",cellfields=["u"=>uHh],append=false,geo_map=cell_geo_map)
...
cell_geo_map = geo_map_func(ΩH)
writevtk(ΩH,dir*"/restriction",cellfields=["u"=>uhH],append=false,geo_map=cell_geo_map)
