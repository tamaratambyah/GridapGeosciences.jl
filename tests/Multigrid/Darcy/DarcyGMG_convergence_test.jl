...
using Gridap.CellData, Gridap.MultiField, Gridap.Algebra, Gridap.Adaptivity, Gridap.Helpers
using GridapSolvers
...
function darcy_gmg_solver(
  model0::GridapDistributed.DistributedDiscreteModel{2,2},
  p_fe::Int,dir::String,n_gmg_lvls::Int,α::Float64,h::Function,vX::Function,ls=LUSolver(),return_vtk=false)


  # get the ranks to help with storing/saving solution
  ranks = model0.octree_dmodel.parts
  ...
  restrictions = setup_restriction_operators(tests_u,qdegree;mode=:residual,solver=CGSolver(JacobiLinearSolver()))
  ...
  # e_u = sqrt(l2( (uh - u_contra_cf),dΩ))

  i_am_main(ranks) && println(summary(solver_u.log))
  i_am_main(ranks) && println(summary(solver_p.log))
  i_am_main(ranks) && println(summary(solver.log))
  ...
  if return_vtk
    panel_cfs = [uh_proj, ph , u_proj_cf, u_proj_cf-covarient_basis_cf⋅uh, sdiv_cf]
  ...
  n = nc(model)
  dxx = dx(model)
  output = @strdict e_u n dxx p_fe lvl gmg_iters cg_iters kylov_iters α
  i_am_main(ranks) && safesave(datadir(dir_convergence, ("darcy_GMG_nref$(lvl)_p$(p_fe)_alpha$(α).jld2")), output)
...
################################################################################
function main(distribute,nprocs;α=1.0)
  ranks = distribute(LinearIndices((nprocs,)))

  i_am_main(ranks) && println("--START--")
  i_am_main(ranks) && println("Auto conference test: Darcy GMG")

  n_ref_lvls = 2
  ps = [1]
  ls = LUSolver()
  models  = _get_octree_refined_models(ranks,n_ref_lvls,true)
  ...
  dir = datadir("GMGDarcyConvergence_alpha$(α)")
  (i_am_main(ranks) && !isdir(dir)) && mkdir(dir)

  p_convergence_test(ranks,ps,models,darcy_gmg_solver,dir,n_gmg_lvls,α,h,vX,ls,true)
...
with_mpi() do distribute
  main(distribute,1;α=100.0)
end

dir = datadir("GMGDarcyConvergence_alpha100.0")
simName ="convergence"
