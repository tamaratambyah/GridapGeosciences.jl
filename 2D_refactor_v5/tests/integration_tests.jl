...
function u_ambient(p::Int,uαβ::Function) # p = panel id
  function _u(Xp)
    ...
    cache = return_cache(SigmaField(r),X1)
    θϕ1 = evaluate!(cache,SigmaField(r),X1)
...
## change the domain
# cmap = map(x-> InvGnomonicField() ∘ SigmaField(r) ∘ PanelRotationField(rp1_3D[x]), panel_ids)  # ambient -> parametric
cmap = map(x->   PanelRotationField(r1p_3D[x]) ∘ SigmaField(r) ∘ GnomonicField() , panel_ids)  # parametric -> ambient
