...
for manifold_model in models
  ...
  L2_biform(u,v) = ∫( u*v )dΩg
  L2_liform(v) = ∫( rhs*v )dΩg
...
# savefig(plotsdir()*"/L2projection_cubedsphere")
...
plot!(xscale=:log10,yscale=:log10,framestyle=:box,
xlabel="n cells",ylabel="L2(u - uh)",legend=:bottomright)
# plot!(nc,1e-2dx.^4,lw=2,c=:black,label="dx^4")
savefig(plotsdir()*"/L2projection_cubedsphere_fespace")
