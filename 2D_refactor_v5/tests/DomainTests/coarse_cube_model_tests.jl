...
"""
Cube sphere model tests
"""
cube_manifold_model = ManifoldDiscreteModel(coarse_cube_model_3D(1),cube)

num_point_dims(cube_manifold_model)
num_cell_dims(cube_manifold_model)

manifold_grid = get_grid(cube_manifold_model)
topo = get_grid_topology(cube_manifold_model)
...
################################################################################
panel_ids = get_panel_ids(cube_manifold_model)
ambient_model = get_ambient_model(cube_manifold_model)
...
################################################################################
V = TestFESpace(cube_manifold_model,ReferenceFE(raviart_thomas,Float64,1), conformity=:HDiv)
f(x) = VectorValue(x[1],x[2])
uh = interpolate(f ,V)

Ω_parametric = Triangulation(cube_manifold_model)
