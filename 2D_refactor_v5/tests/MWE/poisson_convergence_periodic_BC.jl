...
function plot_error(ns,errs,leginf;ls=:solid)
...
plot()
plot_error(ns,errs,collect(keys(dd)))
plot_error(ns,errs_lagrange,collect(keys(dd));ls=:dash)
...
writevtk(Ω,dir*"/poisson_manufactured_periodic_BC",
        cellfields=["u"=>u, "u0"=>_u],append=false)
