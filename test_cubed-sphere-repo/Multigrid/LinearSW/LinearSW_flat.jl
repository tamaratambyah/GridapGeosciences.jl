...
function vecPerp(u)
  # u   = (u1, u2)
  # u^T = (-u2, u1)
  uT = VectorValue(-u[2],u[1])
  return uT
end



f(x) = u_exact(x) + ω*perp(u_exact)(x) + ∇(p_exact)(x)
...
restrictions = setup_restriction_operators(
  tests_u,qdegree;mode=:residual,solver=CGSolver(JacobiLinearSolver(),verbose=false)
)
...
gmg = GMGLinearSolver(
  ...
  # coarsest_solver = CGSolver(JacobiLinearSolver(),verbose=true),
...
eu = uh-u_exact
sqrt(sum(∫( eu⋅eu )dΩ_error )) # 1e-8

ep = ph-p_exact
sqrt(sum(∫( ep⋅ep )dΩ_error )) # O(1)

using DrWatson
dir = datadir("Multigrid")
!isdir(dir) && mkdir(dir)
writevtk(Ω,dir*"/linear_SW_flat",cellfields= ["uh"=>uh, "ph"=>ph, "eu"=>eu, "ep"=>ep, "u"=>u_exact],append=false)
