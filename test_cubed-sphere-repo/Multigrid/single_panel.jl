...
model0 = CartesianDiscreteModel((-π/4,π/4,-π/4,π/4),(1,1))
model1 = Gridap.Adaptivity.refine(model0)
model2 = Gridap.Adaptivity.refine(model1)
model3 = Gridap.Adaptivity.refine(model2)
models = [model3,model2,model1,model0]

mh = ModelHierarchy(models)

pid = 1
my_func(p::Int) = x-> (π/4 + x[1])*(π/4 - x[1]) + (π/4 + x[2])*(π/4 - x[2])
f = my_func
...
trials = TrialFESpace(tests,f(pid))
solver = LUSolver()
...
for (i,degree) in enumerate(degress)
  ...
  for lev in 1:nlevs-1
    ...
    panel_idsh = [pid for i in 1:num_cells(model_h)]
...
plot!(
  # yscale=:log10,
      ...
      framestyle = :box,
      show=true)
savefig(plotsdir("single_projection_transfer_p$(p_fe)_quad10.png"))
...
plot!(
  # yscale=:log10,
      ...
      framestyle = :box,
      show=true)
savefig(plotsdir("single_interpolation_transfer_p$p_fe.png"))
